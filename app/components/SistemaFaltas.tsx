...
function RegistroFaltas() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await registrarFalta({ nombrePadre, nombreAlumno, diasQueFalto, fechaQueFalto, grado });
    setNombrePadre("");
    setNombreAlumno("");
    setDiasQueFalto(1);
    setFechaQueFalto("");
    ...
    <section className="grid gap-5 lg:grid-cols-[1.1fr_1fr]">
      <article className={cardClassName}>
        <div className="mb-5 flex items-center justify-between">
          ...
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">Formulario</span>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-2 block text-sm font-semibold text-blue-900">Nombre del padre o madre</label>
            <input type="text" placeholder="Ej: Maria Gonzalez" value={nombrePadre} onChange={(e) => setNombrePadre(e.target.value)} className={fieldClassName} required />
          </div>
          <div>
            <label className="mb-2 block text-sm font-semibold text-blue-900">Nombre del alumno</label>
            <input type="text" placeholder="Ej: Juan Gonzalez Lopez" value={nombreAlumno} onChange={(e) => setNombreAlumno(e.target.value)} className={fieldClassName} required />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-2 block text-sm font-semibold text-blue-900">Dias que falto</label>
              <input type="number" value={diasQueFalto} onChange={(e) => setDiasQueFalto(Number(e.target.value))} className={fieldClassName} min="1" required />
            </div>
            <div>
              <label className="mb-2 block text-sm font-semibold text-blue-900">Fecha de la falta</label>
              <input type="date" value={fechaQueFalto} onChange={(e) => setFechaQueFalto(e.target.value)} className={fieldClassName} required />
            </div>
          </div>
          <div>
            <label className="mb-2 block text-sm font-semibold text-blue-900">Grado</label>
            <input type="text" placeholder="Ej: 3 Primaria" value={grado} onChange={(e) => setGrado(e.target.value)} className={fieldClassName} required />
          </div>
          <button type="submit" className="w-full rounded-xl bg-gradient-to-r from-blue-400 to-blue-300 px-4 py-3 text-sm font-semibold text-white transition hover:from-blue-500 hover:to-blue-400 shadow-lg shadow-blue-300/40">
            Guardar falta
          </button>
        </form>
      </article>
      ...
      <article className={cardClassName}>
        <div className="mb-5 flex items-center justify-between">
          ...
          <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">{faltas?.length ?? 0} registros</span>
        </div>
        <div className="max-h-[620px] space-y-3 overflow-y-auto pr-1">
          {faltas && faltas.length > 0 ? faltas.map((falta) => (
            <div key={falta._id} className="rounded-xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100/50 p-4 hover:border-blue-300 hover:shadow-md">
              <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <button onClick={() => eliminarFalta({ id: falta._id })} className="rounded-lg border-2 border-red-200 px-2.5 py-1.5 text-xs font-semibold text-red-600 transition hover:bg-red-50 hover:border-red-300">
                  Eliminar
                </button>
              </div>
              <div className="grid grid-cols-3 gap-3 text-sm text-blue-900">
                <p className="rounded-lg bg-blue-100/60 p-2">
                  <span className="block text-xs font-bold uppercase tracking-wide text-blue-700 mb-1">Grado</span>
                  <span className="font-semibold">{falta.grado}</span>
                </p>
                <p className="rounded-lg bg-blue-100/60 p-2">
                  <span className="block text-xs font-bold uppercase tracking-wide text-blue-700 mb-1">Dias</span>
                  <span className="font-semibold">{falta.diasQueFalto}</span>
                </p>
                <p className="rounded-lg bg-blue-100/60 p-2">
                  <span className="block text-xs font-bold uppercase tracking-wide text-blue-700 mb-1">Fecha</span>
...
function GeneradorMensajes() {
  ...
  const generarMensajeConIA = async () => {
    ...
    try {
      ...
      setMensajeGenerado(data.success ? data.mensaje : "No fue posible generar el mensaje. Intenta nuevamente.");
    } catch (error) {
    ...
    <section className="grid gap-5 xl:grid-cols-[1.05fr_1fr]">
      <article className={cardClassName}>
        <div className="mb-5 flex items-center justify-between">
          ...
          <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">IA asistida</span>
        </div>
        <div className="space-y-4">
          <div>
            <label className="mb-2 block text-sm font-semibold text-blue-900">Selecciona un alumno</label>
            <select value={alumnoSeleccionado} onChange={(e) => setAlumnoSeleccionado(e.target.value)} className={fieldClassName}>
          ...
          {resumenAlumno ? (
            <div className="rounded-xl border-2 border-blue-200 bg-gradient-to-br from-blue-100 to-blue-50 p-4 text-sm text-blue-900">
              ...
              <div className="space-y-2 text-sm">
                <p className="flex justify-between"><span className="font-semibold">Padre/Madre:</span><span>{resumenAlumno.nombrePadre}</span></p>
                <p className="flex justify-between"><span className="font-semibold">Grado:</span><span>{resumenAlumno.grado}</span></p>
                <p className="flex justify-between"><span className="font-semibold">Dias faltados:</span><span className="font-bold text-red-600">{resumenAlumno.totalDiasFaltados}</span></p>
                <p className="flex justify-between"><span className="font-semibold">Total registros:</span><span>{resumenAlumno.totalRegistros}</span></p>
              </div>
            </div>
          ) : null}
          <div>
            <label className="mb-2 block text-sm font-semibold text-blue-900">Tono del mensaje</label>
            <select value={tono} onChange={(e) => setTono(e.target.value as Tono)} className={fieldClassName}>
              <option value="formal">Formal</option>
              <option value="amigable">Amigable</option>
              <option value="serio">Serio</option>
            </select>
          </div>
          <button onClick={generarMensajeConIA} disabled={!alumnoSeleccionado || generando} className="w-full rounded-xl bg-gradient-to-r from-blue-400 to-blue-300 px-4 py-3 text-sm font-semibold text-white transition hover:from-blue-500 hover:to-blue-400 disabled:cursor-not-allowed disabled:from-blue-200 disabled:to-blue-200 disabled:opacity-60 shadow-lg shadow-blue-300/40">
          ...
          {mensajeGenerado ? (
            <div className="space-y-3">
              <div className="max-h-96 overflow-y-auto rounded-xl border-2 border-blue-200 bg-blue-50/50 p-4">
                <pre className="whitespace-pre-wrap font-sans text-sm text-blue-900 leading-relaxed">{mensajeGenerado}</pre>
              </div>
              <button onClick={guardarYCopiar} className="w-full rounded-xl bg-gradient-to-r from-green-400 to-green-300 px-4 py-3 text-sm font-semibold text-white transition hover:from-green-500 hover:to-green-400 shadow-lg shadow-green-300/40">
      ...
      <article className={cardClassName}>
        <div className="mb-5 flex items-center justify-between">
          ...
          <span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">{mensajesGuardados?.length ?? 0} items</span>
        </div>
        <div className="max-h-[700px] space-y-3 overflow-y-auto pr-1">
          {mensajesGuardados && mensajesGuardados.length > 0 ? mensajesGuardados.map((msg) => (
            <div key={msg._id} className={`rounded-xl border-2 p-4 transition ${msg.enviado ? "border-green-200 bg-gradient-to-br from-green-50 to-green-100/40 hover:border-green-300" : "border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100/40 hover:border-blue-300"}`}>
              <div className="mb-3 flex items-start justify-between gap-3">
                <div>
                  <p className="font-semibold text-blue-900">{msg.nombreAlumno}</p>
                  <p className="text-sm text-blue-700">Para: {msg.nombrePadre}</p>
                  <p className="text-xs text-blue-600">
                    {new Date(msg.fechaGeneracion).toLocaleDateString("es-MX", { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                  </p>
                </div>
                <span className={`rounded-full px-3 py-1.5 text-xs font-bold whitespace-nowrap ${msg.enviado ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
                  {msg.enviado ? "Enviado" : "Pendiente"}
                </span>
              </div>
              <p className="mb-4 max-h-24 overflow-hidden text-sm text-blue-900 line-clamp-3">{msg.mensaje}</p>
              <div className="flex flex-wrap gap-2">
                <button onClick={() => copiarMensaje(msg.mensaje)} className="rounded-lg border-2 border-blue-200 px-3 py-1.5 text-xs font-semibold text-blue-700 transition hover:bg-blue-100 hover:border-blue-300">
                  Copiar
                </button>
                {!msg.enviado ? (
                  <button onClick={() => marcarEnviado({ id: msg._id })} className="rounded-lg border-2 border-green-200 px-3 py-1.5 text-xs font-semibold text-green-700 transition hover:bg-green-100 hover:border-green-300">
                    Marcar como enviado
                  </button>
                ) : null}
              </div>
    ...
    <section className="mx-auto w-full max-w-4xl">
      <article className={cardClassName}>
        ...
        <div className="mb-4 max-h-[520px] space-y-3 overflow-y-auto rounded-xl border-2 border-blue-200 bg-blue-50/50 p-4">
          {messages.length === 0 ? (
            <p className="text-sm text-blue-700">
              Hola, estoy para ayudarte. Escribe cualquier duda y te respondo de forma clara y amigable.
            </p>
          ) : (
